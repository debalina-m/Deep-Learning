...
def affine_layer(hidden_dim, x):
    ...
    batch_size,features = x.get_shape()
    
    W = tf.get_variable("W", [features,hidden_dim],initializer=tf.contrib.layers.xavier_initializer())
...
def fully_connected_layers(hidden_dims, x):
    ...
    batch_fnn,feature_fnn = x.get_shape()
    ...
    for i in range(len(hidden_dims)):
        ...
        with tf.variable_scope("layer-"+ str(i)) as scope:
            W_fnn = tf.get_variable("W_fnn", shape = [feature_fnn,hidden_dims[i]],initializer=tf.contrib.layers.xavier_initializer())
            b_fnn = tf.get_variable("b_fnn", shape = [1,],initializer=tf.zeros_initializer())

            if i == 0:
                fnn = tf.matmul(x,W_fnn)+b_fnn
            else:
                fnn = tf.matmul(fnn,W_fnn)+b_fnn
            fnn = tf.nn.relu(fnn)
            batch_fnn,feature_fnn = fnn.get_shape()
...
def MakeLogits(x_ph, hidden_dims):
    ...
    '''
    # YOUR CODE HERE
    fcl = fully_connected_layers(hidden_dims, x_ph)
    print(hidden_dims)
    print('fcl.shape-',fcl.shape)
    affine = affine_layer(1,fcl)
    #print('affine.shape', affine.get_shape)
    
   # tf.reshape(affine, [-1])
    #tf.squeeze(affine, [1])
   # affine1 = affine[1]
   # affine1 = tf.squeeze(affine1,[1])
    #print(affine.shape, (tf.squeeze(affine, [1])).shape)
    #print(tf.shape(affine), tf.shape(tf.squeeze(affine, [1])))
    #tf.squeeze(affine, [1])
    #print('after', affine.shape)
    affine1 = tf.squeeze(affine, [1])
    #return -tf.log(1/ (affine1 - 1))
    return affine1
    # END YOUR CODE HERE
...
def MakeLoss(logits, y_ph):
    ...
    loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits = logits, labels = y_ph), name = "loss")
