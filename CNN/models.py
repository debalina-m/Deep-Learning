...
def softmax_output_layer(h_, labels_, num_classes):
    ...
    with tf.variable_scope("Logits"):
        ...
        batch_size, d = h_.get_shape()

        W_out = tf.get_variable('W_out', shape=(d,num_classes), initializer=tf.random_normal_initializer(seed=0))

        b_out = tf.get_variable("b_out", shape = [num_classes,],initializer=tf.zeros_initializer())

        logits_ = tf.add(tf.matmul(h_, W_out), b_out)        
...
def BOW_encoder(ids_, ns_, V, embed_dim, hidden_dims, dropout_rate=0,
    ...
    h_ = tf.math.reduce_sum(xs_,axis=1)

    # Build a stack of fully-connected layers
        
    h_ = fully_connected_layers(h_, hidden_dims)
...
def CNN_encoder(ids_, ns_, V, embed_dim, filters, kernel_sizes, hidden_dims,
    ...
    for kernel_size in kernel_sizes:
        ...
        pool = max_len - kernel_size +1
        c_hat_k_ = tf.layers.conv1d(
                        inputs= xs_,
                        filters = filters,
        ...
        c_hat_k_ = tf.layers.max_pooling1d(c_hat_k_, pool_size= pool, padding='valid', strides=1)

        # Apply dropout
        if dropout_rate > 0: 
            c_hat_k_ = tf.layers.dropout(c_hat_k_,dropout_rate,training=is_training)
        
        c_hat_k_ = tf.math.reduce_sum(c_hat_k_,axis=1)
    ...
    # Build a stack of fully-connected layers
    if hidden_dims:
        h_ = fully_connected_layers(z_, hidden_dims)
    else:
        h_ = z_
